...
The normative reference for Stem syntax. Every implementation
(parser, formatter, LSP, AI prompts) must match it.
...
1.0 ships a built-in registry covering the elements listed above.
...
## 13. Reserved-for-future appendix

Things deliberately out of scope for 1.0, listed here so future
...
5. **Bare `(text)` at top level**: parse error in 1.0. AST stays
   uniform — every block has a name. Implicit-`p` sugar may be added
   in a future version as a non-breaking ergonomic change.
