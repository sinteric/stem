//! Grammar v2 parser. See `docs/grammar-v2.md`.
//!
//! Parallel to the v1 parser in `parser.rs`; lives behind a separate
//! public entry point so the playground can toggle between v1 and v2 on
//! the same source. The v2 parser will become the only parser once the
//! toggle ships and v1 is removed.
//!
...
pub fn parse(src: &str) -> ParseResultV2 {
    let mut p = Parser {
        cur: Cursor::new(src),
        diags: Vec::new(),
    };
