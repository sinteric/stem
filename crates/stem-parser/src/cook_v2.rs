...
/// Output of `cook_document_v2_with`. Cook diagnostics are separate
...
pub fn cook_document_v2(doc: &Document) -> Document {
    cook_document_v2_with(doc, &CookOptions::default()).document
}

/// Cook with explicit options. Use this when you want to supply a
/// `FileLoader` (CLI case) and/or capture cook-time diagnostics.
pub fn cook_document_v2_with(doc: &Document, opts: &CookOptions) -> CookResult {
...
/// Cascading set (v2.0): `bg`, `fmt`, `weight`, `align`, `valign`,
...
sheet{
  ...
  ")
    ...
    use crate::cook_document_v2_with;
    use std::collections::HashMap;
    use std::sync::Mutex;
    ...
    fn source_with_loader_desugars_to_cells() {
        ...
        assert!(r.diagnostics.is_empty(), "{:?}", r.diagnostics);
        let result = cook_document_v2_with(
            &r.document,
            &CookOptions {
    ...
    fn source_without_loader_emits_hint() {
        ...
        let result = cook_document_v2_with(&r.document, &CookOptions::default());
        assert!(result
            .diagnostics
            .iter()
