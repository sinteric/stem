//! Tests for the v2 parser. Cover the locked grammar rules from
//! `docs/grammar-v2.md` and the structural constraints.
...
fn bare_at_ident_is_inline_call_not_prose() {
    // unlike v1, `alert(1)` in prose is literal (no `@`); but `@alert(1)` IS a call
...
fn misplaced_properties_after_body_is_error() {
    let r = parse_v2("p(text)[color:red]");
    assert!(r
        .diagnostics
        .iter()
...
fn bare_property_value_with_colon_is_error() {
    let r = parse_v2("section[at:B2:B4]");
    assert!(r
        .diagnostics
        .iter()
