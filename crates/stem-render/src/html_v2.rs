//! Grammar v2 HTML renderer.
//!
//! Walks a `stem_core::ast_v2::Document` and produces HTML. Doesn't
//! consult any registry — renders by element name, with a generic
//! fallback for unknown blocks (consistent with v1 behavior).
...
pub enum HtmlV2Error {
    #[error("write error: {0}")]
    Write(#[from] std::fmt::Error),
}

/// Adapter so the v2 renderer can implement the existing `Renderer`
/// trait which is generic over the document type. We use a thin
/// wrapper type since the trait's `Document` associated meaning is by
/// concrete type — here we just provide an explicit `render` method
/// (and don't impl `Renderer` for v2 since that trait was tied to v1
/// `Document`).
impl HtmlV2Renderer {
    pub fn render(&self, doc: &Document, theme: &Theme) -> Result<String, HtmlV2Error> {
        ...
        let cooked = stem_parser::cook_document_v2(doc);
...
// Required so the Renderer trait's bound is satisfied if anyone wraps
// it; not used by our pipeline directly because v1 `Document` and v2
// `Document` are different types.
impl Renderer for HtmlV2Renderer {
    type Output = String;
    type Error = HtmlV2Error;
    fn render(
        &self,
        _doc: &stem_core::ast::Document,
        _theme: &Theme,
    ) -> Result<String, Self::Error> {
        Err(HtmlV2Error::Write(std::fmt::Error))
    }
}
        ...
        for r in 0..=max_row {
            ...
            for c in 0..=max_col {
                match by_pos.get(&(c, r)) {
                    Some(cell) => render_sheet_cell(out, cell, theme)?,
                    None => writeln!(
                        out,
...
fn render_sheet_cell(
    ...
    // `fmt` is informational at this layer; we don't actually format the
    // numeric value. A future render pass would apply currency/percent
    // formatting against an evaluated value.
    let fmt = cell.prop_str("fmt").unwrap_or("");
    let body = cell.plain_text().unwrap_or_default();
    write!(out, "<td style=\"{}\" data-fmt=\"{}\">", style, html_attr(fmt))?;
    write!(out, "{}", html_text(&body))?;
    writeln!(out, "</td>")?;
    Ok(())
}
